...
# create settings object corresponding to specified inv
APP_ENV  = os.enviroment.get('APP_ENV', 'Dev')
_current = getattr(sys.modules['config.settings'], '{0]Config'.format(APP_ENV))()

# copy attributes to module for convenience
for atr in [f for f in dir(_current) if not '__' in f]:
	# enviroment can overrride enything
